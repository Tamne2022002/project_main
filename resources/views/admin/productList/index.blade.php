...
@section('js')
    <script type="text/javascript">
        var PERMISSION = @php echo $func->CheckPermissionAdmin(session()->get('user')['id'], 'delete_category')?'"true"':'"false"' @endphp;
    </script>
...
<div class="content-wrapper bg-white">
    ...
    <div class="content">
        <div class="container-fluid pt-3">
            @if ($func->CheckPermissionAdmin(session()->get('user')['id'], 'add_category'))
                <div class="w-100 card card-primary card-outline text-sm">
                    <div class="col-md-6">
                        <a href="{{ route('productList.create') }}" class="btn btn-success m-2">Thêm</a>
                    </div>
                </div>
            @endif
            <div class="w-100 card card-primary card-outline text-sm px-3 py-3">
                <div class="card-title mb-2">Tìm kiếm danh mục</div>
                        ...
                        @if (!$categories->isEmpty())
                            @foreach ($categories as $category)
                                <tr>
                                    ...
                                    <td>
                                        @if ($func->CheckPermissionAdmin(session()->get('user')['id'], 'edit_category'))  
                                        <a href="{{ route('productList.edit', ['id' => $category->id]) }}"
                                            class="btn btn-default">Sửa</a>
                                        @endif
                                        @if ($func->CheckPermissionAdmin(session()->get('user')['id'], 'delete_category'))
